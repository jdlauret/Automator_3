...
word_and_id_file = 'word_and_id.json'
...
def word_check(word, word_num, survey_id, comment_type, obj_key):
    ...
    if word_num < 2:
        ...
        elif new_word in excluded_words['excluded_words']:
            return False
        ...
        for word in text:
            ...
            if valid_word:
                ...
                if word not in word_and_id[survey_id][obj_key][comment_type]['Word_List']:
                    word_and_id[survey_id][obj_key][comment_type]['Word_List'].append(word)
...
def list_generator(data, data_header, word_num, reset=False):
    """
    ...
    for j, line in enumerate(data):
        ...
        if j > 0:
            ...
            enps_comment = line[enps_comment_col]
            ...
            if check_reviewed(key_name, survey_id, 'ENPS') is False:
                if enps_comment is not None:
                    enps_comment = str(enps_comment)
                    comment_handler(survey_id, enps_comment, key_name, word_num, 'ENPS')

                word_and_id[survey_id][key_name]['ENPS']['Reviewed'] = True
