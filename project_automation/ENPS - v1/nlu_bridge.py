...
import watson_developer_cloud.natural_language_understanding.features.v1 \
  as Features
from oracle_bridge.oracle_bridge import run_query, clear_table, update_table
import sys
...
if __name__ == '__main__':
    data = run_query('', '', raw_query='SELECT NSR.SURVEY_ID, NSR.NPS_COMMENTS, NSR.ANPS_COMMENTS '
                                       'FROM JDLAURET.NPS_SURVEY_RESPONSES NSR ORDER BY NSR.SURVEY_ENDED_AT DESC')
    watson_data_storage(data, data[0])
    table_name = 'JDLAURET.NPS_WATSON_DATA'
    clear_table(table_name)
    update_table(table_name, watson_data['analysis'])